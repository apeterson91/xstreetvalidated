...
with hints of how it might be used to inform urban policy making.
...
is the number of neighbors for area $i$ and 0 everywhere else] 
$D$, adjacency matrix^[where $A_{i,j} = 1$ indicates area $i$ and $j$ are 
spatially connected and 0 indicates they are not] $\mathbf{A}$, and spatial 
...
case with spatial applications modeling the counts of disease cases using, e.g.
a Poisson distribution. All of these issues prompted the development of a 
...
shared scale $\sigma$ and correlation parameter $\rho$, that improved 
...
from a poisson distribution. As with disease incidence, city events will also 
...
$Y_{itm}$ be the number of shootings observed at street $i$, during calendar 
year $t$, month $m$ and $\pi$, the probability of being assigned to the zero 
point mass. Using a similar notation as above for the BYM components, 
the model is as follows:

\begin{equation}
\begin{aligned}
Y_{it} \sim \ \pi\delta_{0}(\cdot) + 
...
street (where $b_i = 0$) all else equal. As a final note, you should notice 
how this plot agrees with Figure \@ref(fig:sstreets) which is a nice 
reinforcement that the model is capturing the data correctly.
...
from 2019-2021 --- rather than offering us anything inferential or potentially causal 
...
Thanks to [Robert Svoboda](https://www.linkedin.com/in/rcsvoboda/) for 
