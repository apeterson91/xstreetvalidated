...
[case study](shorturl.at/bvIW2) by Mitzi et al. with less elaboration on 
...
while the third and fourth focus on how this method can be applied and 
used to inform urban policy making.

## The CAR model

The reader may recall from the [previous post](https://github.com/apeterson91/xstreetvalidated/tree/main/_posts/2021-08-26-spatial-models) 
on spatial models, that the CAR family accommodates spatial correlation 
between spatially adjacent observations through the incorporation of a spatial 
effect $\mathbf{b}_i$ whose covariance was a function of the aforementioned 
spatial adjacency. Specifically, for an areal observation $y_i$, a linear model 
with design matrix $\mathbf{X}$ and regression coefficients vector 
$\mathbf{\beta}$, would incorporate CAR effects as follows:

\begin{equation}
\begin{aligned}
y_i &=  \mathbf{X}\mathbf{\beta} + b_i\\
\mathbf{b}_i &\sim MVN(\mathbf{0},\Sigma)\\
\Sigma &= \tau D(I-\rho \mathbf{A}),
\end{aligned}
(\#eq:car)
\end{equation}

where $\Sigma$, the covariance matrix for the random effects $\mathbf{b}_i$ 
are a function of scale $\tau$, difference matrix $D$, adjacency matrix 
$\mathbf{A}$, and spatial correlation $\rho$. When $\rho=1$ the adjacent 
observations are understood to be completely correlated and when fixing 
this parameter as such the model is called the Intrinsic Conditional 
Autoregressive (ICAR) model.
...
not comport with substantive understanding of the data. As for the ICAR model, 
by fixing $\rho=1$, the correlation between observations is understood to be completely driven by 
spatial association which can be problematic in situations where there is 
non-spatial variability or problems with over-dispersion, as is often in the 
case with spatial applications modeling the counts of disease cases using a 
Poisson distribution. All of these issues prompted the development of a 
new approach by Besag, York and Mollie, that strives to take advantage of the 
inherent intuition behind the ICAR model but also overcome it's weaknesses. 
It is to this new framework we now turn our attention, but if the reader is 
interested in learning more about the details underlying the CAR and ICAR 
models they should consult chapter 3 of [@banerjee2003hierarchical], 
...
the two sources of variability --- extra-Poisson site specific variability and 
...
[@riebler2016intuitive] and 
others^[ [@riebler2016intuitive] contains a nice summary of the work leading up
to their model.] to further improve the parameterization 
introduced by BYM by introducing a new constraint, shared scale, $\sigma$ and 
correlation parameter, $\rho$ that improved identifiability and facilitated a 
more intuitive placement of priors on the hyper parameters. Denoting the 
site-specific effect for the $i$th location as $\psi_i$ and the spatial effect 
as $\phi_i$ and their original variants as $\psi_i^{*}$,$\phi_i^{*}$, the new 
...
The application in the following section aspires to show that we do.
...
occur, and enable a more intuitive modeling framework.

### Mapping Crimes to Streets


The police reports documenting crime by the Philadelphia Police Department, 
record a GPS coordinate as the location of the shooting, a single point in space 
...
ntdf <- sfnetworks::as_sfnetwork(pltdf,directed=F)
...
The use of the BYM model is particularly important, as we are often 
interested in modeling the count of events on a particular street and will need
the extra flexibility provided by the random effect $\psi_i$ described above 
to avoid problems with over dispersion in modeling the counts as distributed 
from a poisson distribution. On a similar note, city events will also 
typically by zero-inflated, as events such as shootings, car accidents, etc. do 
not --- typically --- occur at such a high frequency across every single street 
as to receive daily, monthly or possibly even an annual record. Thus, a 
zero point mass, or zero-inflated poisson model, is critical to accurately 
modeling the underlying process from which the data are generated. Let 
$Y_i$ be the number of shootings observed at street $i$ across the 2015 
calendar year and $\pi$, the probability of being assigned to the zero point 
mass. Using a similar notation as above for the BYM components, the model is as 
follows:

\begin{equation}
\begin{aligned}
Y_i \sim \ \pi\delta_{0}(\cdot) + (1-\pi)\text{Pois}(\exp{(\alpha + b_i)}),
\end{aligned}
(\#eq:ukeq)
\end{equation}

where $\delta_{0}(\cdot)$ is the dirac delta function at zero, $\alpha$ is a 
global intercept and $b_i$ is the composite spatial and street specific effect 
as described in \@ref(eq:bym2). 

We can fit this model in the `brms` package after creating the spatial network 
and adjacency matrix from functions in the `sfnetworks`,`tidygraph` and 
`igraph` R packages.
...
Plotting the spatial effects $\phi_i$ in Figure \@ref(fig:bym2plot) we're able 
to observe how the rate of shootings varies spatially across the city streets. 


```{r bym2plot, cache = TRUE, fig.cap="Distribution of estimated log rate ratio, as compared to a typical street, of shootings across city streets in Upper Kensington."}
posterior_samples(fit) %>% 
  ...
  summarise(`50%` = median(Sample),
            `2.5%` = quantile(Sample,0.025),
            `97.5%` = quantile(Sample,0.975)) %>% 
  inner_join(pltdf %>% mutate(street_ix=1:n())) %>% 
  gather(contains("%"),key="Stat",value="StatValue") %>% 
  st_as_sf() %>% 
  ggplot() + 
  geom_sf(aes(color=StatValue)) + 
  theme_void() + 
  scale_color_gradientn(colours = c("black","yellow","red")) + 
  facet_grid(~Stat) + 
  theme(legend.title = element_blank())
```

Need to add in description of model results here as well as uncertainty estimates.

I emphasize here that this model is merely descriptive --- it is describing 
the variability of shootings across the Upper Kensington neighborhood streets 
in 2015 --- rather than offering us anything inferential or potentially causal 
as [@moyer2019effect]'s work sought to do. This is however, as far as I can tell,
the first application of the BYM framework to city street level events, though
...
cleanup effect [@moyer2019effect] estimated in their work. Until then!
