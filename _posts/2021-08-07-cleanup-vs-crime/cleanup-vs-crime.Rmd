...
```{r greening,fig.cap="Graphic demonstration of the different cleaning interventions taken from 'Citywide cluster randomized trial to restore blighted vacant land...' [@Branas2946]"}
...
maintaining the lot." You can see a demonstration of this idea in Figure 
\@ref(fig:greening). 
...
```{r shootings,cache=T,fig.width=14,fig.height=8,fig.cap="No-Officer Involved Shootings in Upper Kensington,Philadelphia in 2015 represented (left) spatially and (right) as a network, as interference effects are typically understood."}
## Switch from shootings to DVPRC Rittenhouse Square Traffic Data
...
were effective: a kernel density estimate and - two, differently sized - buffer 
counts. The author's fit two models of a similar form, estimating the 
...
They also adjusted for correlation  across time, space and within clusters to 
try to adjust for similarity in those measures. The kernel density estimate
model is shown below, but we'll leave a more thorough examination of it and
the buffer models for a later post.
...
While the analysis isn't perfect, the author's certainly put in a lot 
of effort to make their results as defensible as possible. We'll talk 
more about why this is the case in the post focused on causal inference but 
for now, you can look at the consistency of the effect of the 
greening intervention across the two different methods of analysis as a good 
indication that there may be something going on here. Furthermore, 
while there may be those that think a 7% decrease in shootings isn't anything 
to write home about, it's important to keep in mind that (1) their 
are human lives on the line! and (2) this was a fairly 
cheap intervention --- all they did was clean up some abandoned lots! 
That's a good payoff^[See the paper's Conclusion section for healthcare costs 
related to shootings.] for such a small investment and in my book, cause for 
celebration.
