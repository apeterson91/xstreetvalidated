...
I'll leave these last questions for the end of the series, however. To start,
I'll draw from a number of sources to form the foundation of our technical
understanding of housing markets. The first, [@o1996urban], will be a
rudimentary *theoretical* introduction to the topic, drawing on a basic
...
the original market supply curve and the Left Over curve. These changes also
ripple through to the Low Quality housing market via the filtering mechanism;
since there are fewer houses being built, fewer houses will filter down into the
low quality market. This will result in a shift in the overall supply in the low
quality market upward. At the same time, O'Sullivan argues that demand will
increase, as consumers will move into the low quality market since they can't
afford the higher prices in the more restricted high quality market. 

All of these changes happen dynamically --- the animation below shows how the
market price will respond according to the permit restrictions on new
construction.
...
new_slope <- 7 # hack solution - should replace with linear interpolation
...
market_price <- df |> 
  filter(abs(p_supply - p_demand) == min(abs(p_supply - p_demand))) |> 
  pull(p_supply) |> head(1)

df$market_price <- market_price

df_full_restrictions <- tibble(
  q = df$q,
  p_supply = 75 + q * 5,
  p_demand =  350 - q * 5,
  restrictions = "100%",
...
market_price <- df_full_restrictions |> 
  filter(abs(p_supply - p_demand) == min(abs(p_supply - p_demand))) |> 
  pull(p_supply) |> head(1)

df_full_restrictions$market_price <- market_price
...
df_restriction_quarter$market_price <- market_price
  

pltdf <- bind_rows(df, df_full_restrictions, df_restriction_quarter) |> 
   gather(contains("p_"), key = "Supply", value = "Price") |> 
   mutate(cat = case_when(
      str_detect(Supply, "demand") ~ "Demand",
      TRUE ~ "Supply"
  ...
  rbind(pltdf) |> 
  mutate(market_quantity = if_else(market_price == Price, q, 0)) |> 
  group_by(market, restrictions) |> 
  mutate(market_quantity = max(market_quantity)) |> 
  ungroup()
