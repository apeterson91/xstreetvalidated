...
than those further away. We can't know which exact kernel they used
because they don't state it explicitly, but of the many 
[options](https://en.wikipedia.org/wiki/Kernel_(statistics)) 
they may have chosen the Gaussian kernel, as it is popular in 
these settings. For illustrative purposes, you can see it below with bandwidth 
$b$ for location 
$\mathbf{s}=(s_1,s_2)$^[Where $s_1,s_2$ are e.g. latitude and longitude],

$$
\mathcal{K}_b(\mathbf{s}_1,\mathbf{s}_2) = 
\exp(-\frac{1}{2b^2}d(\mathbf{s}_1,\mathbf{s}_2)),
$$
where $d(\mathbf{s}_1,\mathbf{s}_2)$ is the distance between the two locations. 
The KDE,$\hat{y}$ at location $\mathbf{s}$ for a given 
time period (e.g. month) would then be defined as follows:

$$
\hat{y}(\mathbf{s}) = \frac{\sum_{i=1}^{n}\mathcal{K}_b(\mathbf{s}, 
\mathbf{s'})y_i(\mathbf{s'})}{\sum_{i=1}^{n}\mathcal{K}_b(\mathbf{s},
\mathbf{s}')}.
$$
...
weighted exponentially less. The authors create their main figure in the paper
from plotting the relative averages at each grid point across space. The lower
values are assigned a more green color and the higher values are assigned a 
red color. See Figure \@ref(fig:kde) adapted here from their paper[@moyer2019effect].

```{r kde,fig.width=8,fig.height=10,fig.cap="Kernel Density Estimate of shootings adapted from [@moyer2019effect]."}
knitr::include_graphics("Fig1_adapted.jpg")
```
...
been just as good? Further, while the idea of a KDE of shootings can be nicely 
...
so the author's, understandably,  went with something more feasible.
...
300 meters around that center, and counted all the shootings that fell into
that circle, during a given month and assigned that value. 
See Figure \@ref(fig:buffervis) for a visualization.
...
of shootings (or whatever you're counting) within a certain distance of the 
...
300 meters in this case, as the "correct" distance 
within which to count shootings. This remains a major drawback.

<aside>
There have been developments in the statistics literature to try and address these
that we'll discuss in detail at another point in time. See
...
the greening intervention. Similarly, $\beta_6$ is the estimated effect of the 
mowing intervention.
...
incorporating mixed effects. See my post on spatial models or 
[@fitzmaurice2008longitudinal] for a more general introduction on this topic, 
as we'll focus only on those aspects relevant to this application in the 
following section.
...
statistical methods and estimation procedures discussed here.